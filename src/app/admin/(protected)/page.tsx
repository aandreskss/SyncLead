...
export default async function AdminPage({
  ...
  const orgs = await getAllOrgsWithStats(q)
    ...
    <div>
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ fontSize: 13, color: "#71717a", margin: 0 }}>
          {orgs.length} org{orgs.length !== 1 ? "s" : ""}{q ? ` para "${q}"` : " registradas en SyncLead"}
        </p>
      </div>
      ...
      <div style={{ background: "#18181b", border: "1px solid #27272a", borderRadius: 12, overflow: "hidden" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
          ...
          <tbody>
            {orgs.map((org, i) => (
              <tr key={org.id} style={{ borderBottom: i < orgs.length - 1 ? "1px solid #1f1f23" : "none" }}>
            ...
            {orgs.length === 0 && (
              <tr>
                <td colSpan={6} style={{ padding: 40, textAlign: "center", color: "#52525b" }}>
                  {q ? `No hay organizaciones que coincidan con "${q}"` : "No hay organizaciones registradas"}
                </td>
              </tr>
            )}
          </tbody>
