...
export function CreateLeadDialog({ campaigns, defaultCampaignId, onClose }: Props) {
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <label className="block text-xs font-medium text-ops-tx2 mb-1.5">
              Campaña <span className="text-ops-coral">*</span>
            </label>
            <select
              value={campaignId}
              onChange={(e) => setCampaignId(e.target.value)}
              required
              className="w-full rounded-lg border border-ops-bd bg-ops-s2 px-3 py-2 text-sm text-ops-tx focus:outline-none focus:border-ops-blue transition-colors"
            >
