  ...
  return (
    <section className="space-y-4">
      <div className="flex items-center gap-2">
        <MessageCircle className="h-4 w-4 text-ops-tx2" />
        <h2 className="text-sm font-semibold text-ops-tx">Confirmación de WhatsApp</h2>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        >
          <p className={`text-sm font-medium ${config.confirmationMode === "manual" ? "text-ops-green" : "text-ops-tx"}`}>
            Manual
          </p>
          ...
          {config.confirmationMode === "manual" && (
            <p className="text-xs text-ops-green mt-2 font-medium">✓ Seleccionado</p>
          )}
        </button>
        ...
        >
          <p className={`text-sm font-medium ${config.confirmationMode === "provider" ? "text-blue-300" : "text-ops-tx"}`}>
          ...
          {config.confirmationMode === "provider" && (
            <div className="mt-2">
              {config.hasProvider ? (
                ...
                <p className="text-xs text-ops-amber font-medium">⚠ Sin proveedor configurado</p>
              )}
            </div>
          )}
        </button>
      ...
      <div className="flex items-center gap-3">
        ...
        {saved && <p className="text-xs text-ops-green">Guardado</p>}
      </div>
    </section>
  )
}
