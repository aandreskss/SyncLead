      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Code2 className="h-4 w-4 text-ops-blue-t" />
            Script Multi-Campaña
          </DialogTitle>
          {step === "select" && (
            <DialogDescription>
              Selecciona las campañas e ingresa el valor exacto de{" "}
              <code className="text-ops-blue-t bg-ops-blue/10 px-1 rounded">utm_campaign</code>{" "}
        ...
        {step === "select" ? (
          <div className="space-y-4">
            {activeCampaigns.length === 0 ? (
              <p className="text-ops-tx3 text-sm py-4 text-center">
              ...
              <>
                ...
                <div className="flex gap-3 text-xs">
                  <button onClick={selectAll} className="text-ops-blue-t hover:text-ops-blue-t transition-colors">
                    Seleccionar todas
                  </button>
                  <span className="text-ops-tx3">·</span>
                  <button onClick={clearAll} className="text-ops-tx3 hover:text-ops-tx transition-colors">
                    Limpiar
                  </button>
                </div>
                      ...
                      >
                        ...
                        >
                          <div className={`h-4 w-4 rounded border flex items-center justify-center flex-shrink-0 transition-colors ${
                            isSelected ? "bg-ops-blue border-ops-blue" : "border-ops-bd2"
                          }`}>
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              <span className="text-sm font-medium text-ops-tx truncate">{campaign.name}</span>
                              {campaign.client?.name && (
                                <span className="text-xs text-ops-tx3 truncate">{campaign.client.name}</span>
                              )}
                            </div>
                          </div>
                        ...
                        {isSelected && (
                          <div className="px-3 pb-3" onClick={(e) => e.stopPropagation()}>
                            <div className="flex items-center rounded-md border border-ops-bd bg-ops-s1 overflow-hidden focus-within:border-ops-blue transition-colors">
                              <span className="px-2.5 py-1.5 text-xs text-ops-tx3 bg-ops-s2 border-r border-ops-bd whitespace-nowrap font-mono select-none">
                                utm_campaign=
                              </span>
                              <input
                                type="text"
                                ...
                                className="flex-1 px-2.5 py-1.5 text-xs text-ops-tx font-mono bg-transparent focus:outline-none placeholder-ops-tx3"
                              />
                            </div>
                ...
                {selected.size > 1 && !defaultId && (
                  <p className="text-xs text-ops-amber/80 bg-ops-amber/10 border border-ops-amber/20 rounded-lg px-3 py-2">
                ...
                <div className="flex justify-end pt-2 border-t border-ops-line">
                  <button
                    onClick={handleContinue}
                    disabled={!canGenerate || loadingOrganic}
                    className="flex items-center gap-2 px-4 py-2 text-sm bg-ops-blue hover:bg-ops-blue text-white rounded-lg transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                  >
